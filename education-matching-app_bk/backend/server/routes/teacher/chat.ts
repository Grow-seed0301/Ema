...
/**
 * Register teacher chat routes
 */
export function registerTeacherChatRoutes(app: Express): void {
  /**
   * @swagger
   * /teacher/chats:
   *   get:
   *     summary: Get teacher's chat list
   *     description: Retrieve all chats for the authenticated teacher
   *     tags: [Chats]
   *     security:
   *       - bearerAuth: []
   *     responses:
   ...
   */
  app.get("/api/teacher/chats", isAuthenticated, async (req, res) => {
  ...
  /**
   * @swagger
   * /teacher/chats/with/{participantId}:
   *   get:
   *     summary: Get or create chat with a participant
   *     description: Get an existing chat with a participant (student), or create a new one if it doesn't exist
   *     tags: [Chats]
   *     security:
   *       - bearerAuth: []
   *     parameters:
   ...
   *         schema:
   *           type: string
   *         description: ID of the participant (student) to chat with
   *     responses:
   *       200:
   ...
   */
  app.get(
    "/api/teacher/chats/with/:participantId",
    isAuthenticated,
    async (req, res) => {
      await handleGetOrCreateChat(req, res, {
        userType: "teacher",
        getParticipant: storage.getUser.bind(storage),
   ...
   */
  app.get(
    "/api/teacher/chats/:chatId/messages",
    isAuthenticated,
    async (req, res) => {
  ...
  /**
   * @swagger
   * /teacher/chats/{chatId}/read:
   *   post:
   *     summary: Mark messages as read
   *     description: Mark all messages in a chat as read for the authenticated teacher
   *     tags: [Chats]
   *     security:
   *       - bearerAuth: []
   *     parameters:
  ...
  /**
   * @swagger
   * /teacher/chats/{chatId}/image/upload:
   *   post:
