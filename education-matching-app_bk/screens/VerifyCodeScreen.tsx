...
type RouteProps = RouteProp<AuthStackParamList, "VerifyCode">;
...
export default function VerifyCodeScreen() {
  ...
  if (__DEV__ && !route.params?.userRole) {
    console.warn('VerifyCodeScreen: userRole was not provided in route params, defaulting to "student"');
  }
  ...
  const handleVerify = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await apiService.verifyResetCode(email, fullCode, userRole);

      if (response.success) {
        navigation.navigate("ResetPassword", { email, code: fullCode, userRole });
      } else {
        Alert.alert(
          "エラー",
      ...
      >
        ...
        <View style={styles.textSection}>
          <ThemedText style={styles.title}>
            {userRole === 'teacher' ? '教師' : '生徒'}アカウント - コードを確認
          </ThemedText>
          <ThemedText
